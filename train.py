...
def setup_logging(opt):
    ...
    fh = logging.FileHandler(os.path.join(opt.result_dir, 'eval_results/log.txt'))
...
def train(opt, logger):
    ...
    if opt.noamopt:
        ...
        optimizer = utils.get_std_opt(model, d_model=opt.input_encoding_size, factor=opt.noamopt_factor, warmup=opt.noamopt_warmup)
    ...
    while True:
        ...
        # target_labels are one-hot vectors
        optimizer.zero_grad()
        if not sc_flag:
            if opt.label_smoothing == 0:
                loss = crit(dp_model(ocr_feats, ocr_masks, ocr_relation_feat, fc_feats, att_feats, labels, att_masks), target_labels, masks[:,1:])
            else:
                loss = crit(dp_model(ocr_feats, ocr_masks, ocr_relation_feat, fc_feats, att_feats, labels, att_masks), labels[:,1:], masks[:,1:])
        else:
