...
class AttCatModel(CaptionModel):
    def __init__(self, opt):
        super(AttCatModel, self).__init__()
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks, ocr_feats, ocr_masks):
        ...
        return fc_feats, att_feats, p_att_feats, att_masks, ocr_feats, ocr_masks
    ...
    def _forward(self, ocr_feats, ocr_masks, fc_feats, att_feats, seq, att_masks=None):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_ocr_feats, p_ocr_masks = self._prepare_feature(fc_feats, att_feats, att_masks, ocr_feats, ocr_masks)
        ...
        for i in range(seq.size(1) - 1):
            ...
            output, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_ocr_feats, p_ocr_masks, state)
            outputs[:, i] = output
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, ocr_feats, ocr_masks, state):
        ...
        dynamic_ocr_scores = self.ocr_ptr_net(state[0][1], ocr_feats, ocr_masks) # (b, ocr_size)
    ...
    def _sample_beam(self, ocr_feats, ocr_masks, fc_feats, att_feats, att_masks=None, unk_idx=0, ocrunk_idx=0, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                ...
                logprobs, state = self.get_logprobs_state(it, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, tmp_ocr_feats, tmp_ocr_masks, state)
                logprobs[..., unk_idx] = -1e10
                logprobs[..., ocrunk_idx] = -1e10

            self.done_beams[k] = self.beam_search(unk_idx, ocrunk_idx, state, logprobs, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, tmp_ocr_feats, tmp_ocr_masks, opt=opt)
    ...
    def _sample(self, ocr_feats, ocr_masks, fc_feats, att_feats, att_masks=None, unk_idx=0, ocrunk_idx=0, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_ocr_feats, p_ocr_masks, state)
            logprobs[..., unk_idx] = -1e10
            logprobs[..., ocrunk_idx] = -1e10
...
class TopDownCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        prev_h = state[0][-1]
        att_lstm_input = torch.cat([prev_h, torch.mean(att_feats, 1), xt], 1)
...
class TopDownCatModel(AttCatModel):
    def __init__(self, opt):
        super(TopDownCatModel, self).__init__(opt)
        self.num_layers = 2
