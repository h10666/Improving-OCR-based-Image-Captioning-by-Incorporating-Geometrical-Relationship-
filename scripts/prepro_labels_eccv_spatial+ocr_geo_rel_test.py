...
def build_vocab(imgs, img2rel, img2ocr_geo_rel, vocab_num, params):
  ...
  for img in tqdm(data_imgs):
    origin_dup_ocr_tokens = np.load(str(params['input_ocr_root'])+'/'+img['image_id']+'_info.npy',\
                        allow_pickle=True)[()]['ocr_tokens']
    aug_dup_ocr_tokens = np.load(str(params['input_aug_ocr_root'])+'/'+img['image_id']+'_info.npy',\
                         allow_pickle=True)[()]['ocr_tokens']
    dup_ocr_tokens = origin_dup_ocr_tokens + aug_dup_ocr_tokens
    ...
    img['spatial_relations'] = img2rel[img['image_id']][:100+params['max_ocr_len'], :100+params['max_ocr_len']]
    img['ocr_geo_relations'] = img2ocr_geo_rel[img['image_id']][:params['max_ocr_len'], :params['max_ocr_len']]
...
def main(params):
  ...
  img2ocr_geo_rel = {}
  img2ocr_rel_v = {}
  for i in range(1,params['subtask_num']+1):
    rel_dicts = pickle.load(open(params['input_spatial_rel_pkl']+str(i)+'.pkl', 'rb'))
    img2rel.update(rel_dicts['img2rel'])
    img2ocr_geo_rel.update(rel_dicts['img2ocr_geo_rel'])
  print(len(img2rel))

  seed(123) # make reproducible
  
  # create the vocab
  # vocab, top20, all_imgs = build_vocab(imgs, img2rel, img2ocr_geo_rel, params)
  # itow = {i+1:w for i,w in enumerate(vocab)} # a 1-indexed vocab translation table
  # wtoi = {w:i+1 for i,w in enumerate(vocab)} # inverse table
  
  out = pickle.load(open('../../data/step5_3_eccvrel+mmrel_google+base/TC.pkl','rb'))
  itow = out['ix_to_word']
  wtoi = {itow[key]:key for key in itow.keys()}
  all_imgs = build_vocab(imgs, img2rel, img2ocr_geo_rel, len(wtoi)+1, params)
  ...
  #     vocab_f.write(itow[i+1]+'\n')
  # pickle.dump(itow, open(params['output_vocab']+'.pkl', 'wb'))
  # input()
  ...
  for i,img in enumerate(all_imgs):
    ...
    jimg['spatial_relations'] = img['spatial_relations']
    jimg['ocr_geo_relations'] = img['ocr_geo_relations']
...
if __name__ == "__main__":
  ...
  parser.add_argument('--input_obj_root', required=True, help='')
  parser.add_argument('--input_spatial_rel_pkl', required=True, help='')
  parser.add_argument('--input_aug_ocr_root', default=None, required=True, help='')
